...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    std::unique_lock<std::shared_mutex> w_lock(mu_);

    std::unique_ptr<TrieNode> *cur = &root_;

    for (uint32_t i = 0; i < key.size(); ++i) {
      if (i < key.size() - 1 && (*cur)->HasChild(key[i])) {
        cur = (*cur)->GetChildNode(key[i]);
      } else if (i < key.size() - 1) {
        (*cur)->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
        cur = (*cur)->GetChildNode(key[i]);
      } else if (i == key.size() - 1) {
        if ((*cur)->HasChild(key[i]) && (*(*cur)->GetChildNode(key[i]))->IsEndNode()) {
          return false;
        } else if ((*cur)->HasChild(key[i])) {
          (*cur)->InsertChildNode(
              key[i], std::make_unique<TrieNodeWithValue<T>>(std::move(**(*cur)->GetChildNode(key[i])), value));
          return true;
        } else {
  ...
  bool Remove(const std::string &key) {
    ...
    uint32_t i = 0;
    std::unique_ptr<TrieNode> *cur = &root_;
    std::stack<std::unique_ptr<TrieNode> *> temp;

    for (; i < key.size(); ++i) {
      if ((*cur)->HasChild(key[i])) {
        temp.push(cur);
        cur = (*cur)->GetChildNode(key[i]);
        if (i == key.size() - 1 && !(*cur)->IsEndNode()) {
          return false;
        }
      } else {
        return false;
    ...
    std::unique_ptr<TrieNode> *pre = cur;
    while (!temp.empty()) {
      pre = temp.top();
      temp.pop();
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    for (uint32_t i = 0; i < key.size(); ++i) {
      if ((*cur)->HasChild(key[i])) {
        ...
        if (i == key.size() - 1) {
          if ((*cur)->IsEndNode()) {
            auto result = dynamic_cast<TrieNodeWithValue<T> *>((*cur).get());
            if (result != nullptr) {
              *success = true;
